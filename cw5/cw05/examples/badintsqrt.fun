...
def addfive(n: Int): String = n + 5.0;
...
// hint: change the second count to 'ms'
def write_ellipses(count: Double, countt: Int): Void = {
    if count == 0 
    then ...
    else {
        spinsleep(ms);
...
// hint: change the return type to void
def idbg(tag: String, n: Int): Bool = {
    write(tag);
    write(" = ");
    write(n, false);
    write('\n', true);
