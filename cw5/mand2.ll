...
@Ymin = constant double -1.3

@Ymax = constant double 1.3

@Ystep = constant double 0.05

@Xmin = constant double -2.1

@Xmax = constant double 1.1

@Xstep = constant double 0.02

@Maxiters = constant i32 1000

define fastcc void @m_iter(i32 %m, double %x, double %y, double %zr, double %zi) {
    %tmp.0 = load i32, ptr @Maxiters
    %tmp.1 = tail call i1 @builtin.icmp.sle(i32 %tmp.0, i32 %m)
    br i1 %tmp.1, label %bb.3, label %bb.2

bb.3:
    %tmp.6 = add i32 0, 32
    tail call void @print_char(i32 %tmp.6)
    br label %bb.4

bb.2:
    %tmp.8 = fadd double 0.0, 4.0
    %tmp.9 = tail call double @builtin.fmul(double %zi, double %zi)
    %tmp.10 = tail call double @builtin.fmul(double %zr, double %zr)
    %tmp.11 = tail call double @builtin.fadd(double %tmp.9, double %tmp.10)
    %tmp.12 = tail call i1 @builtin.fcmp.ule(double %tmp.8, double %tmp.11)
    br i1 %tmp.12, label %bb.14, label %bb.13

bb.14:
    %tmp.17 = add i32 0, 48
    %tmp.18 = add i32 0, 10
    %tmp.19 = tail call i32 @builtin.irem(i32 %m, i32 %tmp.18)
    %tmp.20 = tail call i32 @builtin.iadd(i32 %tmp.17, i32 %tmp.19)
    tail call void @print_char(i32 %tmp.20)
    br label %bb.15

bb.13:
    %tmp.22 = add i32 0, 1
    %tmp.23 = tail call i32 @builtin.iadd(i32 %m, i32 %tmp.22)
    %tmp.24 = tail call double @builtin.fmul(double %zr, double %zr)
    %tmp.25 = tail call double @builtin.fadd(double %x, double %tmp.24)
    %tmp.26 = tail call double @builtin.fmul(double %zi, double %zi)
    %tmp.27 = tail call double @builtin.fsub(double %tmp.25, double %tmp.26)
    %tmp.28 = fadd double 0.0, 2.0
    %tmp.29 = tail call double @builtin.fmul(double %tmp.28, double %zr)
    %tmp.30 = tail call double @builtin.fmul(double %tmp.29, double %zi)
    %tmp.31 = tail call double @builtin.fadd(double %tmp.30, double %y)
    tail call void @m_iter(i32 %tmp.23, double %x, double %y, double %tmp.27, double %tmp.31)
    br label %bb.15
...
define fastcc void @x_iter(double %x, double %y) {
    %tmp.0 = load double, ptr @Xmax
    %tmp.1 = tail call i1 @builtin.fcmp.ule(double %x, double %tmp.0)
    br i1 %tmp.1, label %bb.3, label %bb.2

bb.3:
    %tmp.6 = add i32 0, 0
    %tmp.7 = fadd double 0.0, 0.0
    %tmp.8 = fadd double 0.0, 0.0
    tail call void @m_iter(i32 %tmp.6, double %x, double %y, double %tmp.7, double %tmp.8)
    %tmp.10 = load double, ptr @Xstep
    %tmp.11 = tail call double @builtin.fadd(double %x, double %tmp.10)
    tail call void @x_iter(double %tmp.11, double %y)
    br label %bb.4
...
define fastcc void @y_iter(double %y) {
    %tmp.0 = load double, ptr @Ymax
    %tmp.1 = tail call i1 @builtin.fcmp.ule(double %y, double %tmp.0)
    br i1 %tmp.1, label %bb.3, label %bb.2

bb.3:
    %tmp.6 = load double, ptr @Xmin
    tail call void @x_iter(double %tmp.6, double %y)
    %tmp.8 = add i32 0, 10
    tail call void @print_char(i32 %tmp.8)
    %tmp.10 = load double, ptr @Ystep
    %tmp.11 = tail call double @builtin.fadd(double %y, double %tmp.10)
    tail call void @y_iter(double %tmp.11)
    br label %bb.4
...
define fastcc void @main() {
    %tmp.0 = load double, ptr @Ymin
    tail call void @y_iter(double %tmp.0)
    ret void
}
